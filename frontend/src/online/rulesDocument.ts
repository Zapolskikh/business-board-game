...
import { automationCost, influencePerPoint, marketRerollCost, moneyPerPoint, projectPerkText, projectRequirementText, rarityLabels } from "./gameUi";
...
const roleGuides: Record<string, RoleGuide> = {
  capitalist: {
    ...
    powers: [{
      ...
      use: "Инвестиционное действие можно потратить на покупку объекта, дополнительного слота, замену объекта или жетон автоматизации. Оно не подходит для продажи, развития района, покупки карты или другого обычного действия.",
    }],
  ...
  <main>
    <section id="goal">
      ...
      <ul>
        ...
        <li><b>Объекты</b> — половина цены объекта, округление вниз. Дорогие и редкие карточки дают больше очков.</li>
      ...
      <div class="tip"><b>Практика:</b> городской заказ (+2$) даёт всего 0.2 очка за действие — это худший ход в игре, годный лишь чтобы добрать монеты до покупки. Кампания (2$ → 2◆) выгоднее, потому что влияние идёт в проекты. Ваш текущий счёт с разбивкой всегда виден в панели «Мой счёт».</div>
    </section>
    ...
    <section id="flow">
      ...
      <p><b>Обычные действия</b> тратятся на что угодно: город, роли, карты, серые операции, защиту, покупки. <b>Инвестиционные действия</b> — только на покупку объектов, слотов, замену объекта и жетон автоматизации.</p>
    ...
    <section id="projects">
      ...
      <ul>
        ...
        <li><b>Ротация</b> — в начале каждого раунда <b>первый слева</b> проект уходит <b>под низ колоды</b> (не из игры — он вернётся позже) и заменяется новым; уходящий помечен значком ⏳. За 3$ без действия доску можно обновить самому, раз в ход.</li>
    ...
    <section id="resources">
      <h2>3. Ресурсы</h2>
      <div class="cols">
        <div class="box"><h4>💵 Деньги ($)</h4><p>Топливо: покупки, замена объектов, жетон автоматизации, реролл рынка, деньги в цене проектов. В финале лишь ${moneyPerPoint(meta)}$ = 1 очко, поэтому копить бессмысленно — тратьте.</p></div>
        <div class="box"><h4>💎 Влияние (◆)</h4><p>Нужно для ролей, городских проектов и многих способностей. В финале ${influencePerPoint(meta)}◆ = 1 очко. Копится медленно, зато его сложнее отнять.</p></div>
        ...
        <div class="box"><h4>🔄 Реролл рынка</h4><p>${marketRerollCost(meta)}$ и <b>без действия</b>, один раз за ход: все шесть позиций рынка объектов меняются. Способ превратить лишние деньги в нужную покупку.</p></div>
      </div>
    </section>
    ...
    <section id="economy">
      ...
      <h3>♻ Замена объекта</h3>
      <p>За <b>одно действие</b> любой ваш объект меняется на объект с рынка: возврат <code>⌊цена/2⌋</code> зачитывается в цену нового, доплачивается только разница. Жетон автоматизации переезжает на новый объект. Слоты при этом не освобождаются и не занимаются.</p>
      <p>Обычная продажа объекта тоже осталась: 1 действие, возврат <code>⌊цена/2⌋</code>, слот освобождается.</p>
    ...
    <section id="cards">
      ...
      <p>Покупка карты: 3$ + 1◆, <b>действие не расходуется</b> — за ход можно докупить карты до предела руки, были бы деньги и влияние. В руке максимум 3 карты. Розыгрыш бесплатный, но <b>только одна карта за ход</b> — это и есть основной ограничитель. Ненужную карту можно конвертировать без затраты действия: <code>+1$</code> (продать) или <code>+1◆</code> (сбросить). Рынок карт полностью обновляется каждый раунд.</p>
      <table>
