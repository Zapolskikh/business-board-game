...
import { campaignTiers, patronage, lobbying, crisisPrInfluence, marketRotationSize, projectPerkText, projectRequirementText, projectRerollMoney, rarityLabels } from "./gameUi";
...
const roleGuides: Record<string, RoleGuide> = {
  capitalist: {
    ...
    powers: [],
    advice: "Роль без активных способностей и целиком про темп: каждый объект приносит на доллар больше, скидка помогает открыть новые районы, а деловые связи закрывают условия объектов.",
  },
  politician: {
  ...
  <main>
    <section id="goal">
      ...
      <ul>
        ...
        <li><b>Объекты</b> — половина цены объекта, округление вниз; это число написано прямо на карточке (<b>N очк</b>). Через объект деньги превращаются в очки по <b>2$ за очко</b> — лучший курс в игре, и потому объекты и есть главный сток денег.</li>
        ...
        <li><b>Деньги и влияние сами по себе очков не дают вовсе.</b> Это топливо: всё, что осталось в кошельке на последнем ходу, стоит ноль. Единственный выход из накоплений — патронаж, лоббирование, объекты и проекты, и все они стоят действий.</li>
