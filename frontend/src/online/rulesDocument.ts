...
const roleGuides: Record<string, RoleGuide> = {
  ...
  journalist: {
    ...
    perks: [
      "<b>Доход от повестки.</b> В конце раунда Журналист получает <b>1$ за каждый скандал</b>, который на этот момент числится у соперников, и <b>2$</b>, если у вас есть хотя бы один активный объект Делового центра. Считается остаток, а не полученное за раунд: снятые до расчёта скандалы не приносят ничего.",
      "<b>Собственный рейтинг.</b> В конце раунда каждый собственный скандал даёт +1◆, но не больше <b>2◆ плюс 1◆ за каждый ваш активный объект Спального района</b>: тираж растёт вместе с читателями.",
      "<b>Предел скандалов 6.</b> Роль теряется на шестом скандале, а не на пятом; тюрьма по-прежнему на следующем.",
      "У Журналиста <b>нет профильного района</b> — вместо него две связи с чужими: Деловой центр удваивает деньги, Спальный поднимает потолок влияния.",
    ],
    powers: [
      {
      ...
      {
        name: "Публикация",
        cost: "1 обычное действие и 3◆",
        limit: "1 раз за ход, отдельно от Раздуть скандал",
        effect: "Выбранный соперник получает <b>2 скандала</b>.",
    ...
    warning: "Крыша соперника может полностью поглотить публикацию. При 6 собственных скандалах Журналист теряет роль и перестаёт получать её бонусы в конце раунда, при 7 — попадает под арест.",
  },
  fraudster: {
    ...
    powers: [
      ...
      },
      {
        ...
        limit: "1 раз за ход",
        effect: "Забирает <b>25% денег каждого соперника</b> (Крыша цели отменяет изъятие у неё) и даёт вам <b>5 скандалов</b> — весь скандальный бюджет роли: без подготовки вы теряете роль в тот же миг, а с двумя перками на снижение скандалов остаётся 3 и роль выживает. Скандалов начисляется ровно столько, сколько указано в выбранном числе, независимо от того, сколько денег удалось собрать, — за вычетом эффектов снижения скандалов.",
  ...
  mafia: {
    ...
    powers: [
      ...
      {
        name: "Замять дело",
        cost: "1 обычное действие, 3$; нужен активный административный объект",
        limit: "по числу действий",
  ...
  military: {
    ...
    powers: [{
      ...
      use: "Санкция — единственный способ снять роль без компромата, и он не требует влияния. Держите цель под наблюдением: игрок, который сам себе вешает скандалы серыми операциями, к третьему-четвёртому раунду отдаёт вам и деньги, и влияние, и роль.",
    }],
  ...
  <main>
    ...
    <section id="projects">
      ...
      <ul>
        ...
        <li><b>Городские инициативы</b> — отдельная строка под доской: всегда доступны и брать их можно <b>сколько угодно раз</b>, но каждая уже взятая делает следующую дороже на <b>${meta.scoring?.initiative_surcharge_influence ?? 1}◆ и ${meta.scoring?.initiative_surcharge_money ?? 2}$</b>. Цена на карточке инициативы всегда показана уже с вашей надбавкой. Это выход из мёртвой руки, а не стратегия: к пятой-шестой инициативе лоббирование становится выгоднее.</li>
      </ul>
    ...
    <section id="resources">
      <h2>3. Ресурсы</h2>
      <div class="cols">
        <div class="box"><h4>💵 Деньги ($)</h4><p>Топливо: покупки, слоты, оба реролла, деньги в цене проектов, кампания, отмывание и карты, покупающие очки напрямую. В финале лишь ${moneyPerPoint(meta)}$ = 1 очко, поэтому копить невыгодно — тратьте. Аварийный сток вдвое выгоднее хранения: патронаж, ${patronage(meta).money}$ → ${patronage(meta).points} очка, раз за ход.</p></div>
        <div class="box"><h4>💎 Влияние (◆)</h4><p>Нужно для ролей, городских проектов и многих способностей. В финале ${influencePerPoint(meta)}◆ = 1 очко. Аварийный сток вдвое выгоднее: лоббирование, ${lobbying(meta).influence}◆ → ${lobbying(meta).points} очка, раз за ход — но проект платит примерно очко за 1◆, так что лоббирование это floor, а не план. Само по себе почти не растёт: его дают объекты с «+◆ за раунд», постоянные бонусы проектов, роль Политика и Журналиста. Остальное приходится покупать за действия — кампанией или отмыванием.</p></div>
        <div class="box"><h4>🎖️ Патронаж</h4><p>1 обычное действие и <b>${patronage(meta).money}$ → ${patronage(meta).points} очка</b> в «Прочие очки». Слот не нужен, карта не нужна, <b>один раз за ход</b>. Курс ${patronage(meta).money / patronage(meta).points}$ за очко — <b>худший в игре</b>: объект даёт очко за 2$, проект — примерно за 1$. Это сток для денег, которые уже некуда девать, и страховка от партии, где доход вырос, а доска проектов пуста.</p></div>
        ...
        <div class="box"><h4>🛡️ Крыша</h4><p>Заряд защиты <b>от чужих действий</b>: гасит направленную на вас карту, рэкет, санкции, взлом и попытку отобрать роль. Последствия ваших собственных решений — провал вашей серой операции без страховки — Крыша не отменяет.</p></div>
    ...
    <section id="economy">
      ...
      <p><code>доход = развитая база + синергии</code>, где <code>развитая база = ⌈…⌈база × 1.25⌉ … ⌉</code> — по одному округляемому вверх шагу за каждый уровень развития района.</p>
      <ul>
        <li><b>База</b> — доход, напечатанный на карточке объекта.</li>
        <li><b>Развитие района</b> даёт +25% за уровень к базовому доходу <b>только ваших</b> объектов этого района — уровень личный, соперники от него ничего не получают. Максимум 2 уровня. Каждый уровень округляется вверх отдельно, поэтому даже объект с доходом 1–2$ получает минимум +1$ за уровень. Развитие покупается прямо на районе: <b>звёзды ★☆ в таблице «Районы и рынок» — это кнопка</b>, и в подсказке к ней написано, сколько именно даст следующий уровень.</li>
    ...
    <section id="districts">
      ...
      <div class="tip"><b>Практика:</b> ранняя специализация на 1–2 районах быстрее раскручивает синергии и профильную роль. Но полный «моно-район» (4 объекта) уязвим к карте «Антимонопольное расследование» и точечным блокировкам — держите запасной доход.</div>
    </section>
    ...
    <section id="grey">
      ...
      <p>Серые операции открывает <b>район</b>, а не отдельная карточка: достаточно любого активного объекта нужного района, роль не требуется. Каждая операция стоит 1 обычное действие, имеет базовый шанс успеха и может быть застрахована Крышей на случай провала. Серый сектор открывает все пять; Технокластер — «Памп и дамп» и «Взлом»; Административный квартал — «Слив компромата».</p>
      <table>
        <thead><tr><th>Операция</th><th>Район-ключ</th><th>Шанс</th><th>Эффект при успехе</th><th>Скандалы</th></tr></thead>
        <tbody>
          <tr><td class="name"><b>Отмывание</b></td><td>Серый сектор</td><td class="num">65%</td><td>(${meta.scoring?.laundering_base_cost ?? 4} + ⌊раунд/2⌋)$ → (${meta.scoring?.laundering_base_gain ?? 2} + ⌊раунд/3⌋)◆ <b>и +3 очка</b></td><td>+1 (провал: остальные +2)</td></tr>
          <tr><td class="name"><b>Контрабанда</b></td><td>Серый сектор</td><td class="num">55%</td><td>украсть до (3 + ⌊раунд/2⌋)$ у цели <b>и +3 очка</b></td><td>+1 (провал: остальные +2)</td></tr>
          <tr><td class="name"><b>Памп и дамп</b></td><td>Технокластер, Серый сектор</td><td class="num">45%</td><td>+(6 + раунд)$, лидер −(2 + ⌊раунд/2⌋)$ <b>и +3 очка</b></td><td>+1 (провал: остальные +3)</td></tr>
          <tr><td class="name"><b>Взлом</b></td><td>Технокластер, Серый сектор</td><td class="num">40%</td><td>украсть до ${meta.scoring?.hack_influence_steal ?? 4}◆ у цели <b>и +5 очков</b></td><td>+2 (провал: остальные +3)</td></tr>
          <tr><td class="name"><b>Слив компромата</b></td><td>Серый сектор, Административный квартал</td><td class="num">70%</td><td>${meta.scoring?.compromat_influence ?? 3}◆ → цель теряет роль (раз в раунд) <b>и +5 очков</b></td><td>+2 (провал: остальные +3)</td></tr>
        </tbody>
      </table>
      <p><b>Отмывание — единственный неограниченный способ превратить деньги во влияние.</b> Денег в партии всегда больше, чем есть куда их девать, а влияние приходится покупать действиями, поэтому серый сектор существует именно ради этого обмена. Обе стороны растут с раундом, и растут по-разному: курс отмывания в любом раунде <b>лучше</b>, чем у кампании — за это вы платите скандалом и слотом под серый объект. «Взлом» и «Слив компромата» бьют по влиянию и по роли: денежная атака против игрока с сотней в кармане ничего не значит.</p>
      <p><b>Слив компромата</b> снимает с цели роль целиком: это −3 очка, весь пассив роли и освободившееся место, которое теперь занимается по свободной цене, а не по цене переворота. Доступен только раз в раунд, требует роли у цели, и его гасит Крыша.</p>
      <p><b>Успешная операция приносит очки:</b> +3 за обычную и +5 за «Взлом» и «Слив компромата» — те две, что стоят двух скандалов. Провал не приносит ничего, поэтому серый сектор — это размен риска и репутации на прямой счёт, а не только на деньги.</p>
      <p>Аферист повышает шанс успеха на 30 п.п. без всяких условий и получает влияние за отставание в рейтинге. Некоторые серые объекты («офшор») дополнительно снижают скандалы.</p>
    ...
    <section id="scandals">
      ...
      <p>Чистка скандалов — <b>одна кнопка</b> в панели решений, и она подставляет цену вашей роли. Каждый вариант стоит 1 обычное действие, лимита по числу применений нет:</p>
      <div class="grid">
