...
const roleGuides: Record<string, RoleGuide> = {
    ...
    powers: [
      {
        ...
        cost: "1 действие и 1 скандал",
        limit: "метка одна: новая снимает предыдущую",
        effect: "Карта на рынке считается вашей: приносит доход, входит в синергию района, засчитывается в условия проектов и открывает серые операции своего квартала. При этом она <b>остаётся в продаже по обычной цене</b>, очков в финале не даёт и слот не занимает.",
        use: "Метку видит весь стол, и любой может купить помеченную карту у вас из-под носа — это и есть её цена. Ставьте на то, что закрывает вам условие проекта или район, а не на самое дорогое. Вместе с ролью метка снимается.",
      },
    ],
    advice: "Роль целиком про темп: покупайте дорогое (объект стоит половину цены в очках), держите пару объектов Промзоны ради влияния, а меткой добирайте недостающий район — она работает, пока карта лежит на рынке.",
  },
  politician: {
    style: "Роль контроля города: стабильно производит влияние, зарабатывает на общей застройке и безопасно снимает скандалы.",
    perks: [
      "<b>Профильный район — Административный квартал.</b> Каждый ваш активный объект Администрации получает +1$ к доходу.",
      "<b>Влияние с жителей.</b> В конце раунда +1◆ за <b>каждый</b> активный жилой объект на столе, включая объекты соперников: чем плотнее застроен город, тем больше он платит. Это единственный пассив в игре, который читает чужие планшеты, и единственный источник влияния роли — плоского влияния «за факт роли» у Политика нет.",
    ],
    powers: [
      {
        ...
        use: "Очистку выгодно использовать до действия, способного довести вас до 5 скандалов. Повторять можно, пока хватает действий и влияния.",
      },
      {
        name: "Договоримся",
        cost: "3◆ и 1 скандал; нужен свой объект Серого сектора",
        limit: "1 раз за ход, действие не тратится",
        effect: "Любой район на ваш выбор до конца раунда считается вашим: открывает проекты и серые операции этого квартала и входит в синергию.",
        use: "Действия не стоит, поэтому ход остаётся под проекты. Чистый Политик договориться не может: сделку заключают в Сером секторе, нужен свой объект оттуда.",
      },
      {
        name: "Право вето",
        cost: "1 действие и 3◆",
        limit: "вето одно, его видит весь стол",
        effect: "Проект на доске закрыт для всех, кроме вас. Уходит с доски по обычному расписанию и уносит вето с собой.",
        use: "Это покупка темпа на одной карте, а не заморозка доски: держите вето на проекте, условие под который вы соберёте через ход-другой. Вместе с ролью вето снимается.",
      },
    ],
    advice: "Политик особенно силён в длинной партии: город застраивается жильём, и каждый чужой жилой объект тоже платит вам влияние — то самое, которым покупаются проекты. Деньги роль берёт с Администрации, поэтому первый административный объект стоит планировать рано.",
  },
  journalist: {
  ...
  mafia: {
    ...
    perks: [
      ...
      "<b>Крыша.</b> Мафиози платит за Крышу на 1$ меньше остальных (цена растёт с раундом), а хранить может <b>две вместо одной</b>. Единственная роль, у которой защита переживает первый же удар.",
    ],
    powers: [
      {
        name: "Рэкет",
        ...
        limit: "1 раз за ход",
        effect: "Выбранный соперник передаёт деньги и влияние. Деньги: <b>2$ + 2$ за каждый ваш объект Серого сектора</b> + ⌊раунд/3⌋, и <b>+5$, если цель лидирует</b>. Влияние: <b>1◆ за каждый ваш объект Административного квартала</b>. Оба требования ограничены тем, что у цели есть.",
      ...
      },
      {
        ...
        cost: "1 Крыша",
        limit: "1 раз за ход, действие не тратится",
        effect: "Слот рынка до конца раунда закрыт для всех, кроме вас. Снять метку нельзя — её убирает только смена раунда.",
        use: "Плата не действием, а защитой: вы меняете свою неуязвимость на карту, которую иначе перекупят. Метку видит весь стол, поэтому она ещё и говорит соперникам, за чем вы копите.",
      },
    ],
    advice: "Рэкет — главный денежный источник роли, поэтому Серый сектор растите вглубь: каждый его объект добавляет 2$ к взысканию. Административный объект нужен вторым — он же убирает скандал от рэкета и открывает «Замять дело». Метку тратьте на дорогую карту, до которой вам не хватает одного раунда.",
  },
  military: {
    ...
    perks: [
      ...
      "Силовик наказывает по факту: санкция читает счётчик скандалов цели, а «Проверка» этот счётчик создаёт — по скандалу каждому, кто торгует в Сером секторе.",
    ],
    powers: [
      {
        name: "Санкции",
      ...
      },
      {
        name: "Проверка",
        cost: "1 действие",
        limit: "по числу действий; нужен соперник с объектом Серого сектора",
        effect: "По скандалу каждому сопернику, у которого есть объект Серого сектора. Крыша отвечает за своего владельца и тратится.",
        use: "Способность бьёт по району, а не по игроку, поэтому отсидеться тихо нельзя. Это и есть заготовка под санкцию: она требует цель, у которой уже есть скандалы, а проверка их создаёт.",
      },
      {
        name: "Отобрать Крышу",
        cost: "1 действие и 3◆",
        limit: "по числу действий; нужен соперник с Крышей и место под неё у вас",
        effect: "Одна Крыша соперника переходит к вам. <b>Крыша от этого не защищает</b> — иначе способность нельзя было бы применить к тем, у кого она есть.",
        use: "Единственный способ снять чужую защиту, не тратя на неё атаку: Крыша уходит и сразу становится вашей, а цель открывается под санкцию в этом же ходу.",
      },
    ],
