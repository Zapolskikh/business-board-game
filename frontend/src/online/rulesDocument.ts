...
import { automationCost, campaignTiers, crisisPrInfluence, influencePerPoint, marketRerollCost, moneyPerPoint, projectPerkText, projectRequirementText, projectRerollMoney, rarityLabels } from "./gameUi";
  ...
  <main>
    ...
    <section id="projects">
      ...
      <ul>
        ...
        <li><b>Ротация</b> — в начале каждого раунда <b>первый слева</b> проект уходит <b>под низ колоды</b> (не из игры — он вернётся позже) и заменяется новым; уходящий помечен значком ⏳. За ${projectRerollMoney(meta)}$ без действия доску можно провернуть самому, раз в ход — но доска общая, так что ушедший проект исчезает у всех, включая того, кто уже собрал под него условие. Цена в деньгах и высокая специально: влияние — это то, чем сами проекты и покупаются, а дешёвый реролл превращал общую доску в бесконечную раздачу и убивал всякое планирование.</li>
    ...
    <section id="resources">
      ...
      <div class="cols">
        ...
        <div class="box"><h4>🔄 Рероллы</h4><p>Рынок объектов — ${marketRerollCost(meta)}$, доска проектов — ${projectRerollMoney(meta)}$. Оба <b>без действия</b> и по одному разу за ход. Способ превратить лишние деньги в нужную покупку или в живую доску.</p></div>
        <div class="box"><h4>🏷️ Продажа объекта</h4><p><b>Без действия</b>: возврат <code>⌊цена/2⌋</code>, слот сразу свободен. Значит «продать слабый и купить сильный» стоит ровно одно действие покупки.</p></div>
      </div>
    </section>
