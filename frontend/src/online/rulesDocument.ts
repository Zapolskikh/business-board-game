...
import { automationCost, influencePerPoint, marketRerollCost, moneyPerPoint, projectPerkText, projectRequirementText, projectRerollInfluence, rarityLabels } from "./gameUi";
  ...
  <main>
    ...
    <section id="projects">
      ...
      <ul>
        ...
        <li><b>Ротация</b> — в начале каждого раунда <b>первый слева</b> проект уходит <b>под низ колоды</b> (не из игры — он вернётся позже) и заменяется новым; уходящий помечен значком ⏳. За ${projectRerollInfluence(meta)}◆ без действия доску можно провернуть самому, раз в ход — но доска общая, так что ушедший проект исчезает у всех, включая того, кто уже собрал под него условие.</li>
    ...
    <section id="resources">
      <h2>3. Ресурсы</h2>
      <div class="cols">
        <div class="box"><h4>💵 Деньги ($)</h4><p>Топливо: покупки, замена объектов, жетон автоматизации, реролл рынка (общего для всех), деньги в цене проектов. В финале лишь ${moneyPerPoint(meta)}$ = 1 очко, поэтому копить бессмысленно — тратьте.</p></div>
