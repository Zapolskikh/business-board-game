...
import { campaignTiers, patronage, crisisPrInfluence, influencePerPoint, marketRotationSize, moneyPerPoint, projectPerkText, projectRequirementText, projectRerollMoney, rarityLabels } from "./gameUi";
...
const roleGuides: Record<string, RoleGuide> = {
  ...
  journalist: {
    ...
    warning: "Крыша может полностью поглотить начисление. При 6 собственных скандалах Журналист теряет роль и перестаёт получать её бонусы в конце раунда, при 7 — попадает под арест.",
  },
  fraudster: {
...
<div class="layout">
  ...
  <main>
    ...
    <section id="resources">
      <h2>3. Ресурсы</h2>
      <div class="cols">
        <div class="box"><h4>💵 Деньги ($)</h4><p>Топливо: покупки, слоты, оба реролла, деньги в цене проектов, кампания, отмывание и карты, покупающие очки напрямую. В финале лишь ${moneyPerPoint(meta)}$ = 1 очко, поэтому копить бессмысленно — тратьте.</p></div>
    ...
    <section id="roofs">
      ...
      <ul>
        ...
        <li><b>Одна Крыша — вся защита.</b> Отдельных щитов роли и репутации в игре нет: все три карты защиты выдают ту же Крышу, и она гасит любую из трёх атак — попытку перехвата вашей роли (влияние возвращается атакующему), слив компромата и <b>любое начисление скандалов</b> целиком, сколько бы их ни начислялось за раз.</li>
      </ul>
    </section>
