...
type MarketAsset = AssetCard & { uid: string; expiresAtTurn?: number };
type HeldActionCard = ActionCard & { uid: string };
interface Player {
  id: number; name: string; isBot: boolean; difficulty: BotDifficulty; preferredRole: RoleId | null; money: number; influence: number; scandals: number; roofs: number;
...
interface SlotConfig { isBot: boolean; difficulty: BotDifficulty; preferredRole: RoleId | null }
interface GameSettings { roster: SlotConfig[]; maxRounds: number; rolePrice: number }
const BOT_DIFFICULTY_LABELS: Record<BotDifficulty, string> = { easy: "Бот Олега", medium: "Бот Codex", hard: "Бот Claude" };

const DEFAULT_SETTINGS: GameSettings = { roster: [ { isBot: false, difficulty: "medium", preferredRole: null }, { isBot: true, difficulty: "easy", preferredRole: null }, { isBot: true, difficulty: "easy", preferredRole: null }, { isBot: true, difficulty: "hard", preferredRole: null } ], maxRounds: 15, rolePrice: 3 };
...
const initialPlayers = (settings: GameSettings): Player[] => {
  ...
  return settings.roster.map((slot, id) => ({
    ...
    isBot: slot.isBot, difficulty: slot.difficulty, preferredRole: slot.isBot ? slot.preferredRole : null, money: 10, influence: 2, scandals: 0, roofs: 0, role: null,
...
const passiveInfluenceFor = (player: Player) => {
  ...
  // Politician now has a stronger influence engine: every administrative object
  // gives influence (×2 automated), plus a flat civic base of +1◆ and +1◆ for
  // every two residential objects — so жильё тоже питает влияние.
  const administrative = hasRole(player, "politician")
    ? active.filter(a => a.district === "government").reduce((sum, a) => sum + value(a), 0)
      + 1 + Math.floor(active.filter(a => a.district === "residential").length / 2)
    : 0;
...
const effectTotal = (player: Player, key: "extraActions" | "extraInvestmentActions" | "scandalReduction" | "roofCapacity" | "turnRoof" | "maintenanceReduction" | "greyScandalReduction" | "developmentDiscount" | "carryAction") =>
...
const carryActionFor = (player: Player) => Math.min(1, effectTotal(player, "carryAction"));
...
export default function CityPrototype() {
  ...
  const assetAbilities = (asset: OwnedAsset): { text: string; active: boolean }[] => {
    ...
    if (effects?.carryAction) result.push({ text: `Одно неиспользованное действие переносится на следующий ход.`, active: !asset.blocked });
  ...
  const roleBonuses: Record<RoleId, string> = {
    ...
    politician: "Жильё +1$; административные объекты дают влияние; базовые +1◆/раунд и +1◆ за каждые 2 жилых объекта; 4◆ собирают налог района, 2◆ снимают скандал.",
  ...
  const buy = (asset: MarketAsset) => {
    ...
    const replacementDeal = drawMarket(marketDeck, 1, round);
    const replacement = replacementDeal.drawn[0]
      ? { ...replacementDeal.drawn[0], expiresAtTurn: turnSerial + players.length * 2 }
      : null;
    setMarket(xs => [...xs.filter(a => a.uid !== asset.uid), ...(replacement ? [replacement] : [])]);
    setMarketDeck(replacementDeal.rest);
  ...
  const collectDistrictTax = (districtId: DistrictId) => {
    ...
    if (!hasRole(me, "politician") || politicianTaxUsed || me.influence < 4 || revenue < 1) return;
    update(me.id, p => ({ ...p, influence: p.influence - 4, money: p.money + revenue }));
    setPoliticianTaxUsed(true);
    say(`${me.name} собирает налог с района «${DISTRICTS.find(d => d.id === districtId)?.title}»: −4 влияния, +${revenue}$.`);
  };
  ...
  const mafiaRacket = (targetId?: number) => {
    ...
    // Racket power now scales with the mafia's own footprint AND the game clock:
    // Серый сектор и Спальный район усиливают требование, Госсектор — отжатое влияние.
    // С ходами рэкет растёт (+1$ за 2 раунда), а лидер платит больше.
    const isLeader = tid === scores[0]?.id;
    const moneyDemand = 3 + districtCount(me, "shadows") + districtCount(me, "residential") + mafiaOperationBonus + Math.floor(round * 2 / 3) + (isLeader ? 3 : 0);
  ...
  const buyCard = (selected: ActionCard) => {
    ...
    const card = actionCard(selected);
    update(me.id, p => ({ ...p, money: p.money - 3, influence: p.influence - 1, hand: [...p.hand, card] }));
    setActionMarket(cards => cards.filter(c => c.id !== selected.id));
  ...
  const rotateExpiredObjects = (nextTurnSerial: number, forRound = round) => {
    const expired = market.filter(a => a.expiresAtTurn !== undefined && a.expiresAtTurn <= nextTurnSerial);
    if (!expired.length) return;
    const { drawn, rest } = drawMarket(marketDeck, expired.length, forRound);
    const replacements = drawn.map(a => ({ ...a, expiresAtTurn: nextTurnSerial + players.length * 2 }));
    const expiredIds = new Set(expired.map(a => a.uid));
    setMarket(xs => [...xs.filter(a => !expiredIds.has(a.uid)), ...replacements]);
    setMarketDeck(rest);
    pushLog("system", `Рынок объектов: удалено залежавшихся карт ${expired.length}, открыто новых ${replacements.length}.`);
  };
  const rotateActionMarket = () => {
    const available = shuffle([...actionDeck, ...actionMarket]);
    setActionMarket(available.slice(0, 3));
    setActionDeck(available.slice(3));
  };
  ...
  const startGame = () => {
    const roster = settings.roster.slice(0, 6).map(s => ({
      isBot: !!s.isBot,
      difficulty: (["easy", "medium", "hard"] as BotDifficulty[]).includes(s.difficulty) ? s.difficulty : "medium" as BotDifficulty,
      preferredRole: s.isBot && ROLES.some(r => r.id === s.preferredRole) ? s.preferredRole : null,
    }));
    ...
    setMarket(initialDeal.drawn.map(a => ({ ...a, expiresAtTurn: normalized.roster.length * 2 }))); setMarketDeck(initialDeal.rest); setEvent(events[0]);
    setActionMarket(cards.slice(0, 3)); setActionDeck(cards.slice(3)); processingCards.current.clear();
    setDistrict("business"); setTarget(null); setViewPlayerId(null); setFinished(false); setLogExportStatus(""); setAntitrustActive(false); setTurnSerial(0);
  ...
  const preparePlayerTurn = (player: Player) => {
    let changed = { ...player, copiedRole: player.pendingRole, pendingRole: null, jailTurns: Math.max(0, player.jailTurns - 1), turns: player.turns + 1, bankedActions: 0 };
  ...
  const startPlayerTurn = (nextTurn: number) => {
    ...
    const nextTurnSerial = turnSerial + 1;
    rotateExpiredObjects(nextTurnSerial);
    setTurnSerial(nextTurnSerial);
    setPlayers(ps => ps.map(p => p.id === next.id ? prepared : p));
    const actionLimit = next.jailTurns > 0 ? 1 : (prepared.role === "fraudster" ? 4 : 3) + actionBonusFor(prepared) + (next.bankedActions ?? 0);
  ...
  const endTurn = () => {
    const carry = actionsLeft > 0 && me.jailTurns === 0 && carryActionFor(me) > 0 ? 1 : 0;
    if (turnsTakenInRound < players.length - 1) {
      setPlayers(ps => ps.map(p => p.id === me.id ? { ...p, copiedRole: null, bankedActions: carry } : p)); setTurnsTakenInRound(x => x + 1); if (carry) pushLog("consequence", `${me.name} откладывает 1 неиспользованное действие на следующий ход.`); startPlayerTurn((turn + 1) % players.length); return;
    }
    ...
    const nextStart = gameStartingTurn; const { settled, summaries } = settleRound(players, players[nextStart].id);
    const starter = preparePlayerTurn(settled[nextStart]);
    const nextTurnSerial = turnSerial + 1;
    setPlayers(settled.map(p => p.id === starter.id ? starter : p.id === me.id ? { ...p, bankedActions: carry } : p));
    const actionLimit = settled[nextStart].jailTurns > 0 ? 1 : (starter.role === "fraudster" ? 4 : 3) + actionBonusFor(starter) + (settled[nextStart].bankedActions ?? 0);
    rotateExpiredObjects(nextTurnSerial, round + 1); rotateActionMarket(); setTurnSerial(nextTurnSerial); setRound(round + 1); setTurn(nextStart); setTurnsTakenInRound(0); setTarget(null); setActionsLeft(actionLimit); setTurnActionLimit(actionLimit); setFraudTurnPlace(Math.max(1,scores.findIndex(p=>p.id===players[nextStart].id)+1)); resetTurnFlags(investmentBonusFor(starter)); setAntitrustActive(false);
    const starterRole = ROLES.find(r => r.id === starter.role)?.title ?? "без роли";
    setLog(xs => [
  ...
  // If a player is jailed during their own turn (scandals reach 6), the rest of
  // their actions burn immediately and the turn ends automatically.
  const jailEndedRef = useRef(false);
  useEffect(() => {
    if (!gameStarted || finished || me.isBot || me.jailTurns === 0) { jailEndedRef.current = false; return; }
    if ((actionsLeft > 0 || investmentActions > 0) && !jailEndedRef.current) {
      jailEndedRef.current = true;
      setActionsLeft(0); setInvestmentActions(0);
      pushLog("consequence", `${me.name} арестован прямо в свой ход — оставшиеся действия сгорают, ход завершается.`);
      const timer = window.setTimeout(() => endTurn(), 1000);
      return () => window.clearTimeout(timer);
    }
  }, [me.jailTurns, actionsLeft, investmentActions, turn, gameStarted, finished]);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      ...
      // early, so they are scaled by this planning horizon.
      const horizon = Math.max(1, settings.maxRounds - round + 1);

      // ── Difficulty tuning ──────────────────────────────────────────────
      // Profiles are ordered by measured strength: easy is Oleg's original
      // hand-tuned bot, medium is the mechanics-first Codex planner, and hard
      // is Claude's aggressive long-horizon profile.
      const hard = me.difficulty === "hard";
      const codex = me.difficulty === "medium";
      const hzPlan = Math.min(horizon, codex ? 10 : hard ? 8 : 5); // useful income horizon
      const econThreshold = codex ? 0 : hard ? 0.4 : 1;            // net value over a city order
      const cardBuyThreshold = codex ? 5.5 : hard ? 5 : 6;
      const attackTempo = codex ? 5 : hard ? 4 : 6;
      ...
      const preferredRole = me.preferredRole;
      let strategicRole = preferredRole ?? [...strategicRoles].sort((a, b) => roleUtility(b) - roleUtility(a))[0];
      if (!preferredRole && isComebackPosition && !codex) {
      ...
      const cardUtility = (card: ActionCard) => {
        ...
        const attackPhase = Math.min(1, round / attackTempo);
        ...
        if (card.kind === "roof") return me.roofs < roofLimitFor(me) ? preferredRole && me.role === preferredRole ? 8 : 5 : -10;
        ...
        if (card.kind === "role_shield") return preferredRole && me.role === preferredRole ? 8 : me.role ? 5 : 2;
      ...
      if (!cardPlayedThisTurn) {
        ...
        if (playable && (!codex || cardUtility(playable) >= 1)) {
      ...
      if (hasRole(me, "politician") && !politicianTaxUsed && me.influence >= 4) {
        const bestTaxDistrict = [...DISTRICTS].sort((a, b) => objectsInDistrict(b.id) - objectsInDistrict(a.id))[0];
        if (objectsInDistrict(bestTaxDistrict.id) >= 4) { collectDistrictTax(bestTaxDistrict.id); return; }
      }
      if (hasRole(me, "capitalist") && !rolePowerUsed && me.influence >= 3 && me.money >= 4 && (!codex || horizon > 1)) { rolePower(); return; }
      ...
      if (actionsLeft > 0 && me.scandals < 5) {
        ...
        const baseThreshold = round >= settings.maxRounds - 1 ? 12 : (codex ? 5 : hard ? 2 : 3);
        ...
        const shouldChangeRole = me.role !== strategicRole && (preferredRole !== null || me.role === null || roleUtility(strategicRole) >= currentUtility + switchThreshold || (isComebackPosition && round < settings.maxRounds - 1));
        if (shouldChangeRole) {
          ...
          if (!preferredRole && holder && holder.id !== me.id && me.role !== "journalist" && !journalistHolder && me.influence >= settings.rolePrice) { claimRole("journalist"); return; }
        }
      }
      ...
      if (actionsLeft > 0 && me.money >= 3 && me.influence >= 1 && me.hand.length < 3) {
        ...
        if (bestActionCard && purchaseUtility(bestActionCard) >= cardBuyThreshold) { buyCard(bestActionCard); return; }
      }
      ...
      const marketValue = (asset: MarketAsset) => {
        ...
        if (codex) {
          // Project only effects that can turn into score before the game ends.
          // Rarity is deliberately absent: it has no scoring value by itself.
          const futureCount = count + 1;
          const districtIncome = futureCount >= 4 ? 2 : futureCount >= 2 ? 1 : 0;
          const roleDistrict = ROLES.find(r => r.id === strategicRole)?.districts.includes(asset.district) ? 1 : 0;
          const linked = (d: DistrictId) => hasDistrictLink(me, d) || asset.district === d;
          const effects = asset.effects;
          let conditionalIncome = effects?.eventBonus?.eventId === event.id ? effects.eventBonus.value : 0;
          if (effects?.districtBonus) {
            const b = effects.districtBonus;
            conditionalIncome += b.perObject
              ? Math.max(0, districtCount(me, b.district) + (asset.district === b.district ? 1 : 0) - (b.excludeSelf && asset.district === b.district ? 1 : 0) + (b.virtualRole === strategicRole ? 1 : 0)) * b.value
              : linked(b.district) ? b.value : 0;
          }
          if (effects?.roleBonus?.role === strategicRole) conditionalIncome += effects.roleBonus.value;
          for (const rb of effects?.roleBonuses ?? []) if (rb.role === strategicRole) conditionalIncome += rb.value;
          for (const dl of effects?.districtLinks ?? []) if (linked(dl.district)) conditionalIncome += dl.value;
          const passiveInfluence = effects?.influenceBonus
            && (!effects.influenceBonus.role || effects.influenceBonus.role === strategicRole)
            && (!effects.influenceBonus.district || linked(effects.influenceBonus.district)) ? effects.influenceBonus.value : 0;
          const maintenanceSaved = Math.min(effects?.maintenanceReduction ?? 0, me.assets.length + 1);
          const recurring = asset.income + districtIncome + roleDistrict + conditionalIncome + passiveInfluence + maintenanceSaved - 1;
          const actionEngine = (effects?.extraActions ?? 0) * 2.5 + (effects?.extraInvestmentActions ?? 0) * 2
            + (effects?.carryAction ?? 0) * 1.5;
          const defence = (effects?.scandalReduction ?? 0) * (me.scandals >= 2 ? 2 : .75)
            + (effects?.turnRoof ?? 0) * (myPlace === 1 ? 1.5 : .75)
            + (effects?.greyScandalReduction ?? 0) * (me.assets.filter(a => a.tags.includes("grey")).length + 1)
            + (preferredRole ? (effects?.takeoverCompensation ?? 0) * 1.5 : 0);
          const purchase = effects?.purchase;
          const immediate = asset.influence + (purchase?.money ?? 0) + (purchase?.influence ?? 0)
            + assetValue({ ...asset, automated: false, scaled: false, blocked: false, uid: asset.uid })
            + (purchase?.roofs ? 1.5 : 0) + (purchase?.card ? 2 : 0);
          const scandalCost = (purchase?.scandals ?? (asset.tags.includes("grey") ? 1 : 0)) * (me.scandals >= 3 ? 5 : 2);
          return immediate + (recurring + actionEngine + defence) * hzPlan - priceOf(asset) - scandalCost - 2;
        }
        // Hard bots value synergy completion and standalone economy engines more
        // and factor passive-influence objects into the score.
        const completion = hard
          ? (count === 1 ? 6 : count === 3 ? 8 : count === 2 ? 3 : 0)
          : (count === 1 ? 5 : count === 3 ? 7 : count === 2 ? 2 : 0);
        ...
        const specialistRoleMatches = preferredRole
          ? (asset.effects?.roleBonuses ?? []).filter(rb => rb.role === preferredRole).reduce((sum, rb) => sum + rb.value * 3, 0)
            + (asset.effects?.takeoverCompensation ?? 0) * 1.5
          : 0;
        ...
        const strategicEffect = (asset.effects?.extraActions ?? 0) * 16 + (asset.effects?.extraInvestmentActions ?? 0) * 10
          ...
          + (asset.effects?.roofCapacity ?? 0) * 3 + (asset.effects?.turnRoof ?? 0) * 4 + (asset.effects?.greyScandalReduction ?? 0) * 5 + (asset.effects?.carryAction ?? 0) * 9;
        const greyPenalty = asset.tags.includes("grey") && !hasRole(me, "fraudster") && !hasRole(me, "mafia") ? me.scandals * 2 + 2 : 0;
        const influenceEngine = hard ? (asset.effects?.influenceBonus?.value ?? 0) * 4 : 0;
        return asset.income * 2.5 - priceOf(asset) + asset.influence + completion + roleMatch + exactRoleMatch + specialistRoleMatches + conditionMatch + rarityValue + strategicEffect + influenceEngine - greyPenalty;
      };
      ...
      if (actionsLeft > 0 && me.money >= 2 && developable) {
        ...
        const developValue = districtIncome * 0.25 * hzPlan - 2 + (me.assets.some(a => a.district === developable.id) ? 2 : 0);
      ...
      if (canInvest && automation && objectSynergyIncome(me, automation, event) > 0 && me.money >= improvementCost("automate")) {
        econMoves.push({ value: objectSynergyIncome(me, automation, event) * hzPlan - improvementCost("automate"), run: () => improve(automation.uid, "automate") });
      }
      if (canInvest && modernization && me.money >= improvementCost("scale")) {
        econMoves.push({ value: 2 * hzPlan - improvementCost("scale"), run: () => improve(modernization.uid, "scale") });
      }
      if (actionsLeft > 0 && me.influence >= 3) {
        ...
        const strategicOwner = roleHolder(strategicRole);
        const roleReserve = strategicOwner && strategicOwner.id !== me.id ? settings.rolePrice * 3 : me.role ? 0 : settings.rolePrice;
        const spareInfluence = me.influence - roleReserve;
        const projectValue = codex ? 3 + (horizon <= 3 ? 2 : 0) - (spareInfluence >= 3 ? 0 : 6)
          : 6 - (spareInfluence >= 3 ? 0 : 5) - (horizon > 4 ? 2 : 0);
        econMoves.push({ value: projectValue, run: () => cityProject() });
      }
      const bestEcon = econMoves.sort((a, b) => b.value - a.value)[0];
      if (bestEcon && bestEcon.value >= econThreshold) { bestEcon.run(); return; }
      ...
      const strategicRoleHolder = roleHolder(strategicRole);
      const neededRoleInfluence = preferredRole
        ? strategicRoleHolder?.id === me.id ? 0 : roleCost(strategicRoleHolder)
        : strategicRoleHolder ? settings.rolePrice * 3 : settings.rolePrice;
      if (actionsLeft > 0 && me.influence < neededRoleInfluence && me.money >= 2) { basicAction("campaign"); return; }

      // Hard bots defend a lead: cap scandals and insure the leader with a Roof
      // before wasting the turn on a plain city order.
      if ((hard || codex || preferredRole) && actionsLeft > 0) {
        if (me.scandals >= (codex ? 4 : 3) && me.money >= 4) { crisisPR(); return; }
        const protectingTargetRole = preferredRole !== null && me.role === preferredRole;
        if ((myPlace === 1 || protectingTargetRole) && me.roofs < roofLimitFor(me) && me.money >= (hasRole(me, "mafia") ? 2 : 3) + (protectingTargetRole ? 4 : 6)) { buyRoof(); return; }
      }
  ...
  const addSlot = (isBot: boolean) => setSettings(s => s.roster.length >= 6 ? s : ({ ...s, roster: [...s.roster, { isBot, difficulty: "medium" as BotDifficulty, preferredRole: null }] }));
    ...
    <section className="city-setup">
      ...
      <div className="roster-editor">
        ...
        <ul className="roster-list">{roster.map((slot, i) => { const label = slot.isBot ? `Бот ${++botN}` : `Игрок ${++humanN}`; return <li className={`roster-slot ${slot.isBot ? "is-bot" : "is-human"}`} key={i}>
          ...
          {slot.isBot
            ? <><select value={slot.difficulty} onChange={e => updateSlot(i, { difficulty: e.target.value as BotDifficulty })}><option value="easy">Easy · Олег</option><option value="medium">Medium · Codex</option><option value="hard">Hard · Claude</option></select><select aria-label={`Целевая роль: ${label}`} value={slot.preferredRole ?? ""} onChange={e => updateSlot(i, { preferredRole: (e.target.value || null) as RoleId | null })}><option value="">Любая роль</option>{ROLES.map(role => <option value={role.id} key={role.id}>{role.icon} {role.title}</option>)}</select></>
  ...
  return <div className="city-game">
    ...
    {finished ? <section className="city-finish"><h2>Итоги города</h2>{scores.map((p, i) => <p key={p.id}>{i + 1}. <b>{p.name}</b> — {p.score} очков</p>)}<h3>Полный журнал</h3><p className="dim">Сохранено записей: {log.length}. В файле действия расположены от начала игры к завершению.</p><div className="log-export-actions"><button className="btn" onClick={copyGameLog}>Копировать лог</button><button className="btn" onClick={downloadGameLog}>Скачать .txt</button></div>{logExportStatus && <p className="log-export-status">{logExportStatus}</p>}<button className="btn primary" onClick={() => setGameStarted(false)}>Новая партия</button></section> : <main className="city-layout">
      ...
      {!viewingOther && <section className="city-cards action-group card-section g-cards"><h3 className="group-title">🃏 Карты <span className="group-hint">3$ + 1◆ + действие · резерв {actionDeck.length}</span></h3><p className="dim card-rule">Рынок перемешивается каждый раунд. Купленные карты до следующего раунда не заменяются. Розыгрыш бесплатный, но только одна карта за ход. Рука {me.hand.length}/3.</p><div className="action-market">{actionMarket.map(c=><button className={`action-card market-action ${c.tone}`} disabled={actionsLeft<1||me.money<3||me.influence<1||me.hand.length>=3} onClick={()=>buyCard(c)} key={c.id}><strong>{c.title}<em>купить</em></strong><small>{c.text}</small></button>)}</div>{me.hand.some(c=>c.kind==="copy_role")&&<label>Роль для мандата <select value={forgedRoleChoice} onChange={e=>setForgedRoleChoice(e.target.value as RoleId)}>{ROLES.map(r=><option value={r.id} key={r.id}>{r.title}</option>)}</select></label>}{me.hand.map(c => { const targeted=c.targeted===true; const eligible=targeted?eligibleTargetsFor(c):[]; return <div className={`hand-card ${c.tone}`} key={c.uid}><button className="action-card" onClick={() => targeted ? requestTarget(`«${c.title}» — против кого?`, eligible, id => playCard(c, id)) : playCard(c)} disabled={!canPlayCard(c)}><strong>{c.title}<em>{targeted?"→ выбрать цель":"→ себе"}</em></strong><small>{c.text}</small></button><div><button onClick={() => convertCard(c,"money")}>Продать +1$</button><button onClick={() => convertCard(c,"influence")}>Сбросить +1◆</button></div></div>})}{cardPlayedThisTurn&&<small className="card-played-note">✓ Карта в этом ходу уже разыграна</small>}</section>}
      </div>
      ...
      <aside className={`city-actions ${me.isBot ? "bot-turn" : ""}`}><div className="actions-head"><h2>🎛️ Решения</h2><div className={`action-tokens ${actionsLeft === 0 ? "spent" : ""}`} title="Осталось действий в этом ходу"><span className="token-label">Действий</span><span className="token-dots">{Array.from({ length: turnActionLimit }).map((_, i) => <i key={i} className={i < actionsLeft ? "on" : "off"} />)}</span><b className="token-count">{actionsLeft}/{turnActionLimit}</b>{investmentActions > 0 && <span className="token-invest" title="Инвестиционные действия: только покупка и улучшение">+{investmentActions} 💼</span>}</div></div>{me.isBot && <p className="bot-action-note">🤖 Бот анализирует рынок и продолжит автоматически.</p>}{actionsLeft === 0 && investmentActions === 0 && !me.isBot && <p className="no-actions">Действия потрачены. Завершите ход.</p>}{investmentActions > 0 && !me.isBot && <p className="investment-note">Доступно дополнительное действие: покупка объекта, слота, автоматизация или модернизация.</p>}{turnActionLimit===1&&<p className="no-actions">Тюрьма: в этом ходу доступно только одно действие.</p>}
        ...
        <div className="action-group g-roles"><h3 className="group-title">🏷️ Роли <span className="group-hint">свободная {settings.rolePrice}◆ · переворот {settings.rolePrice * 3}◆</span></h3><div className="role-market">{ROLES.map(r => { const holder=roleHolder(r.id);const cost=roleCost(holder);return <button disabled={holder?.id===me.id || me.influence<cost || actionsLeft<1 || me.scandals>=5} onClick={() => claimRole(r.id)} style={{borderColor:r.color}} key={r.id}><span className="role-line"><span className="role-icon" style={{borderColor:r.color}}>{r.icon}</span>{r.title} · {cost}◆</span><small>{holder ? `занята: ${holder.name}${holder.roofs ? " · защищена Крышей" : ""}` : r.passive}</small></button>})}</div>
          ...
          {hasRole(me,"politician") && <div className="politician-powers"><small>Прогноз влияния: +{passiveInfluenceFor(me)}◆ за раунд</small><button className="btn full-width" disabled={politicianTaxUsed||me.influence<4||selectedDistrictObjects<1} onClick={()=>collectDistrictTax(district)}>{politicianTaxUsed?"✓ Налог собран":`Налог: 4◆ → ${selectedDistrictObjects}$ (${selectedDistrictOwnObjects} своих + ${selectedDistrictObjects-selectedDistrictOwnObjects} чужих)`}</button><button className="btn full-width" disabled={politicianCleanupUsed||me.influence<2||me.scandals<1} onClick={cleanPoliticianScandal}>{politicianCleanupUsed?"✓ Скандал урегулирован":"Урегулировать скандал: 2◆"}</button></div>}
          ...
          {hasRole(me,"mafia") && <div className="role-powers mafia"><small className="tribute-note">💰 Дань в конце раунда: в каждом районе, где вы единолично держите большинство, соперники с меньшим числом объектов платят вам 2$ за каждый свой объект в этом районе. Прогноз: до <b>{mafiaTributePotential}$</b> в этом раунде.</small><button className="btn danger" disabled={mafiaRacketUsed||actionsLeft<1||opponents.length===0||!me.assets.some(a=>a.district==="shadows"&&!a.blocked)} title="Отбирает деньги (и влияние при наличии Административного объекта). Сумма растёт с районами Серого/Спального сектора и с ходом партии; лидер платит +2$. Крыша цели отменяет рэкет." onClick={() => requestTarget("Рэкет против кого?", opponents, mafiaRacket)}>{mafiaRacketUsed?"✓ Рэкет проведён":`Рэкет: до ${3 + districtCount(me,"shadows") + districtCount(me,"residential") + mafiaOperationBonus + Math.floor(round*2/3)}$ (+3$ с лидера)`}</button><button className="btn danger" disabled={mafiaRoofSweepUsed||actionsLeft<1||me.roofs<1} onClick={mafiaSweepRoofs}>{mafiaRoofSweepUsed?"✓ Связи сожжены":"Сжечь связи: все теряют Крышу"}</button><button className="btn" disabled={mafiaCleanupUsed||me.roofs<1||me.scandals<1} title="Требуется хотя бы 1 Крыша и 1 скандал. Снимает до 2 скандалов, тратя одну Крышу." onClick={()=>mafiaCleanup("roof")}>Крыша → −2 скандала</button><button className="btn" disabled={mafiaCleanupUsed||me.money<3||districtCount(me,"government")<1||me.scandals<1} title="Коррупция доступна только при наличии объекта Административного квартала, 3$ и хотя бы 1 скандала. Снимает до 2 скандалов за 3$." onClick={()=>mafiaCleanup("money")}>Коррупция: 3$ → −2 скандала{districtCount(me,"government")<1?" 🔒 нужен админ-объект":""}</button></div>}
