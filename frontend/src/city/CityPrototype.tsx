...
export default function CityPrototype() {
  ...
  const rolePower = () => {
    ...
    if (me.role === "mafia" && target !== null) update(target, p => p.money >= 2 ? { ...p, money: p.money - 2 } : { ...p, scandals: p.scandals + 1 });
    if (me.role === "military" && target !== null) update(target, p => ({ ...p, blockedAsset: p.assets[0]?.id ?? null }));
  ...
  const playCard = (card: ActionCard) => {
    ...
    const targeted = ["deal", "scandal", "fine", "steal"].includes(card.kind);
    ...
    say(`${me.name} играет карту «${card.title}»${targeted ? ` — цель: ${targetPlayer?.name}` : " на себя"}.`);
    spendAction();
  };
  ...
  const endTurn = () => {
    ...
    setEventDeck(nextEventDeck); setEvent(nextEvent); setRound(round + 1); setTurn(0); setTarget(null); setActionsLeft(3); setRolePowerUsed(false);
  ...
  return <div className="city-game">
    ...
    <main className="city-layout">
      ...
      <aside className="city-actions"><h2>Решения <span className="action-counter">осталось {actionsLeft}/3</span></h2>{actionsLeft === 0 && <p className="no-actions">Вы потратили все 3 действия. Завершите ход — доход начислится после хода Игрока 4.</p>}<label className={`target-picker ${target === null ? "required" : ""}`}>Цель для направленного действия<select value={target ?? ""} onChange={e => setTarget(e.target.value === "" ? null : Number(e.target.value))}><option value="">— сначала выберите игрока —</option>{players.filter(p => p.id !== me.id).map(p => <option value={p.id} key={p.id}>{p.name}</option>)}</select></label>
        <div className="action-group"><b>Роль</b><div className="role-market">{ROLES.map(r => <button disabled={occupied.has(r.id) || me.influence < 3 || actionsLeft < 1} onClick={() => claimRole(r.id)} style={{borderColor:r.color}} key={r.id}>{r.title}<small>{r.passive}</small></button>)}</div>{me.role && <button className="btn full-width" disabled={rolePowerUsed || actionsLeft < 1 || (["mafia", "military"].includes(me.role) && target === null)} onClick={rolePower}>{rolePowerUsed ? "Полномочие уже использовано" : `Использовать роль: ${role?.power}`}</button>}</div>
        <div className="action-group"><b>Карты в руке</b>{me.hand.map(c => { const targeted = ["deal", "scandal", "fine", "steal"].includes(c.kind); return <button className={`action-card ${c.tone}`} onClick={() => playCard(c)} disabled={actionsLeft < 1 || (targeted && target === null)} key={c.id}><strong>{c.title} <em>{targeted ? `→ ${targetPlayer?.name ?? "выберите цель"}` : "→ себе"}</em></strong><small>{c.text}</small></button>})}</div>
    ...
    <section className="city-help">
      <h2>Как играть — памятка</h2>
      <div className="help-grid">
        <article><h3>🎯 Цель</h3><p>После 10-го раунда набрать больше всех очков: деньги + двойное влияние + стоимость объектов + бонус роли − штраф за скандалы.</p></article>
        <article><h3>⏱️ Ваш ход: 3 действия</h3><p>Купите объект на рынке, возьмите свободную роль за 3 влияния, примените полномочие роли, сыграйте карту, проведите серую схему или купите Крышу.</p></article>
        <article><h3>🏙️ Экономика</h3><p>Купленные объекты приносят доход в конце раунда. Публичное событие сверху заранее показывает, какой район сейчас выгоднее.</p></article>
        <article><h3>🏷️ Роли и влияние</h3><p>Роли уникальны. Влияние нужно для их получения. Полномочие роли используется один раз за ход. При 3 скандалах роль теряется.</p></article>
        <article><h3>🃏 Карты и цели</h3><p>Красные карты атакуют, зелёные создают сделки или доход, голубые защищают. Для направленной карты сначала явно выберите игрока в поле «Цель».</p></article>
        <article><h3>🌒 Риск и Крыша</h3><p>Серые схемы дают быстрые деньги, но при провале создают скандалы. Крыша позволяет владельцу отменить направленный негативный эффект.</p></article>
      </div>
      <p className="help-tip"><b>Первый план:</b> купите доходный объект → накопите 3 влияния → возьмите роль, подходящую вашим районам → используйте карты против лидера, а не автоматически против соседа.</p>
    </section>
  </div>;
}
